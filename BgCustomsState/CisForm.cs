...
{
    public partial class CisForm : Form
    {
        public HtmlDocument htmlDoc;
        public List<CarInfo> list = new List<CarInfo>();
        public List<CarInfo> listTJ = new List<CarInfo>();
        ...
        CarInfo carInfoTJ = new CarInfo();
        public String model = "";
        public String brand = "";
        //创建业务逻辑层对象
        CarInfoBll miBll = new CarInfoBll();
        CicDataBll ciBll = new CicDataBll();
        public CisForm()
        {
            InitializeComponent();

            //IHTMLWindow2 win = (IHTMLWindow2)webBrowser2.Document.Window.DomWindow;
            //string s = window.alert = null; rn window.confirm = null; rn window.open = null; rnwindow.showModalDialog = null; 
            //win.execScript(s, javascript);
            //IEVersion.BrowserEmulationSet();
            //this.webBrowser2.AllowWebBrowserDrop = false;  //可在属性里设置,很重要

            //(this.webBrowser2.ActiveXInstance as SHDocVw.WebBrowser).NewWindow3 += new SHDocVw.DWebBrowserEvents2_NewWindow3EventHandler(Form1_NewWindow3);
            //webBrowser2.NewWindow += new CancelEventHandler(webBrowser2_NewWindow);
            //禁用按钮
        ...
        /// <param name="e"></param>
        private void CisForm_Load(object sender, EventArgs e)
        {
            //設置當前文檔為百度
            //this.webBrowser2.Url = new Uri("http://www.haiguan.info/onlinesearch/gateway/Gatewaystate.aspx");
        ...
        private void  loginStep()
        {
            ...
            htmlDoc = webBrowser2.Document.Window.Frames["leftFrame"].Document;
            if (htmlDoc == null)
            {
        ...
        {
            htmlDoc = webBrowser2.Document.Window.Frames["mainFrame"].Document;
            if (htmlDoc == null)
            {
        ...
        private void AddCarinfo(CarInfo carinfo)
        {
            ...
            htmlDoc.All["CIC_engineNo"].SetAttribute("value", carinfo.Engine_type.Replace("/", ""));
        ...
        private void GetCarinfo(CarInfo carInfo)
        {
            ...
            if (getVinCount >= vinlist.Count)
            {
                ...
                {
                    if (miBll.GetExsitList(getVinList[i]))
                    {
                        bool b = miBll.UpdateCarInfo(getVinList[i]);
                    }
                    else
                    {
                        bool b = miBll.InsertCarInfo(getVinList[i]);
                    }
        ...
        {
            mshtml.IHTMLDocument2 doc = (webBrowser2.ActiveXInstance as SHDocVw.WebBrowser).Document as mshtml.IHTMLDocument2;
        ...
        {
            //自动点击弹出确认或弹出提示
            IHTMLDocument2 vDocument = (IHTMLDocument2)webBrowser2.Document.DomDocument;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            this.webBrowser2.Url = new Uri("https://i.chinaport.gov.cn/deskserver/sw/deskIndex?menu_id=ncic");
            webBrowser2.ScriptErrorsSuppressed = true;
            //等待加载完毕
            while (webBrowser2.ReadyState < WebBrowserReadyState.Complete) Application.DoEvents();
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            carInfoTJ.Batch = batch;
            carInfoTJ.SingleState = "10";
            #region //勾选处理
            dfNoStr = "";
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
            ...
            if (!dfNoStr.Equals(""))
            {
                ...
                listTJ = miBll.selectChecked(carinfo1);
            }
            else
            {
                listTJ = miBll.GetList(carInfoTJ);
        ...
        public void timer5EventProcessor(object source, EventArgs e)
        {
            ...
            if (listTJ.Count==0)
                ...
                //输入报关单号页面查询事件
                if (putInfoForSearch(listTJ[0].Df_no))
                {
                    //解禁按钮
                    setButtonEnable(true);
                    return;
        ...
        public void timer6EventProcessor(object source, EventArgs e)
        {
            ...
            if (rows > 0)
            {
                ...
                {
                    CarInfo carinfo1 = new CarInfo();
                    carinfo1.Batch = carInfoTJ.Batch;
                    carinfo1.SingleState = "8";
                    carinfo1.Df_no = "'" + listTJ[0].Df_no + "'";
        ...
        public void timer7EventProcessor(object source, EventArgs e)
        {
            ...
            if (listTJ.Count > 0)
            {
                listTJ[0].SingleState = "9";
                miBll.UpdateHeadState(listTJ[0]);
            }
            if (listTJ.Count == 0)
            {
                timer7.Stop();
            ...
            lblmessage.Text = listTJ[0].Df_no;
            timer8.Interval = 2000;
            timer8.Start();
            pageFlag = "";
            sum = 1;
